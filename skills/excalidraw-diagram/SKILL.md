...
The `.excalidraw` JSON file is the primary deliverable. Save all diagrams to `~/` (home directory).

It can be:
- Opened directly at [excalidraw.com](https://excalidraw.com) via **File → Open**
- Embedded in web apps using `@excalidraw/excalidraw` React component
- Rendered to PNG via the included Playwright script (see below)

### How to Render (Playwright)

```bash
cd ~/.opencode/skills/excalidraw-diagram/references && uv run python render_excalidraw.py ~/filename.excalidraw
```

This outputs a PNG next to the `.excalidraw` file. Then use the **Read tool** on the PNG to actually view it.

**Requirements:** The renderer needs network access to `esm.sh` to load the Excalidraw library. If the headless browser can't reach esm.sh (firewall, proxy, air-gapped environments), use the alternatives below.

### Alternative Rendering

If Playwright rendering fails:
1. **Open in excalidraw.com** — drag the `.excalidraw` file into the browser, screenshot via DevTools
2. **Use the React component** — embed `<Excalidraw>` in a React app and call `exportToSvg()`
3. **Validate JSON structure** — at minimum, verify the JSON parses and has the right element count
